...
public class JedisDemo {
    public static void start()  {
        ...
        // Add initial nodes
        clusterManager.addNode("192.168.10.53", 6379);
        clusterManager.addNode("192.168.10.53", 6380);
        clusterManager.addNode("192.168.10.53", 6381);
        ...
        if (clusterManager.createCluster()) {
            ...
            if (clusterManager.addNewNodeToCluster("192.168.10.53", 6382)) {
                // Continue operations after adding the node
                redisMap.put("key3", 3);
                System.out.println("key3: " + redisMap.get("key3"));

                // Simulate removing a node
                if (clusterManager.removeNodeFromCluster("192.168.10.53", 6382)) {
